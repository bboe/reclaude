...
- **Everything sortable is sorted.** Functions lexicographically within each half (ASCII order, so `_private` first); dict keys, container items, TOML/YAML keys, and constants wherever order has no semantic meaning. Parameters after the first are keyword-only (`*`), declared and passed in alphabetical order. Workflow YAML keys are fully sorted, including top-level (`jobs`/`name`/`on`) and job-level keys. Deliberate exceptions: `AGE_WINDOWS` (Ctrl-T cycle order), `classify_dir`'s return tuple, span lists, and workflow `steps` (execution order).
